                            ...
                            case 'WORD': {
                                if (!context.includes('FUNCTION::START')) {
                                    if (variables[value_1] !== undefined) {
                                        ...
                                        if (!tokens.slice(parseInt(item_token) + 1).filter(function (x) { return x.token !== 'SPACE'; })[0] ||
                                            tokens.slice(parseInt(item_token) + 1).filter(function (x) { return x.token !== 'SPACE'; })[0].token !== 'ARGUMENTS') {
                                            built_1.push('()');
                                        }
                                    }
                                    ...
                                    else if (prototypes.includes(value_1)) {
                                        built_1.push(value_1);
                                        if (!tokens.slice(parseInt(item_token) + 1).filter(function (x) { return x.token !== 'SPACE'; })[0] ||
                                            tokens.slice(parseInt(item_token) + 1).filter(function (x) { return x.token !== 'SPACE'; })[0].token !== 'ARGUMENTS') {
                                            built_1.push('()');
                ...
                var _loop_3 = function (i) {
                    ...
                    if (context.includes('VARIABLE::CONDITION')) {
                        ...
                        if (context.includes('VARIABLE::CONDITION::ELSE')) {
                            built_1 = built_1.join('').replace(condition, '').replace('%CONDITION%', condition + ' ? ' + variable_value_1.value + ' ').split('');
                            context.splice(context.findIndex(function (x) { return x === 'VARIABLE::CONDITION::ELSE'; }), 1);
                        }
                        else {
